...
from app.recovery_p110 import EVIDENCE_STATE as P110_EVIDENCE_STATE, RecoveryP109ReplayEvidence, _FIELDS as P110_RECEIPT_FIELDS
from app.recovery_p111 import EVIDENCE_STATE, TRUTH_BOUNDARY, _FIELDS, store_p110_replay_identity
...
def _evidence() -> RecoveryP109ReplayEvidence:
    values = {
...
def test_p111_field_contract_extends_exact_p110_serialized_identity():
    assert _FIELDS == (
        *P110_RECEIPT_FIELDS,
...
def test_stores_canonical_verified_p110_identity(tmp_path):
    destination = tmp_path / "nested" / "p110.identity.json"
    stored = store_p110_replay_identity(_evidence(), destination_path=destination)
    ...
    assert stored.p110_evidence_state_verified is True
    assert stored.p110_verification_flags_verified is True
...
def test_storage_is_deterministic_and_replaces_existing_file(tmp_path):
    ...
    first = store_p110_replay_identity(_evidence(), destination_path=destination)
    first_payload = destination.read_bytes()
    second = store_p110_replay_identity(_evidence(), destination_path=destination)
...
def test_rejects_weakened_p110_verification_contract(tmp_path, flag):
    with pytest.raises(ValueError, match="verification flags are incomplete"):
        store_p110_replay_identity(
...
def test_rejects_state_drift_and_authority_escalation(tmp_path):
    with pytest.raises(ValueError, match="state is incompatible"):
        store_p110_replay_identity(
            replace(_evidence(), evidence_state=P110_EVIDENCE_STATE + "_DRIFT"),
            destination_path=tmp_path / "x",
        )
...
def test_stored_identity_is_sensitive_to_every_retained_semantic_field(tmp_path, field):
    ...
    first = store_p110_replay_identity(original, destination_path=tmp_path / "a")
    second = store_p110_replay_identity(
...
def test_rejects_incompatible_evidence_type(tmp_path):
    with pytest.raises(ValueError, match="incompatible type"):
        store_p110_replay_identity(object(), destination_path=tmp_path / "x")


def test_truth_boundary_remains_explicitly_non_authoritative(tmp_path):
    rendered = store_p110_replay_identity(
