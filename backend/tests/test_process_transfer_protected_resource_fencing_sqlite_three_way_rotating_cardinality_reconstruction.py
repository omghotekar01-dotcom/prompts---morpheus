...
# Each identity undergoes one controlled blocked-member loss/reconstruction in a
# round where it has at least two live members, without changing final cardinality.
PERTURBED_GROUP_BY_ROUND = ("b", "a", "c")


def test_three_way_rotating_cardinality_survives_controlled_member_reconstruction(
    tmp_path: Path,
) -> None:
    database = tmp_path / "three-way-rotating-cardinality-reconstruction.sqlite3"
    ...
    for round_index, (group_sizes, perturbed_group) in enumerate(
        zip(CARDINALITY_ROTATION, PERTURBED_GROUP_BY_ROUND, strict=True), start=1
    ):
        pending_counter = BASE_COUNTER + round_index
        pending_version = round_index + 1
        assert group_sizes[perturbed_group] > 1
        ...
        holder = context.Process(
            ...
            name=f"three-way-rotating-reconstruction-holder-{round_index}",
        )
        holder.start()
        ...
        for group, group_size in group_sizes.items():
            for index in range(1, group_size + 1):
                ...
                process, results = _start_waiter(
                    ...
                    value=values[group],
                    label=f"three-way-rotating-reconstruction-{round_index}-{group}-{index}",
                )
        ...
        lost_label = group_labels[perturbed_group][0]
        lost_process = waiters.pop(lost_label)
        result_queues.pop(lost_label)
        group_labels[perturbed_group].remove(lost_label)
        _terminate_waiter(
            lost_process,
            f"round {round_index} blocked member from mutation {perturbed_group}",
        )

        replacement_label = f"{perturbed_group}_replacement"
        replacement, replacement_results = _start_waiter(
            context,
            database,
            counter=pending_counter,
            mutation_id=mutation_ids[perturbed_group],
            value=values[perturbed_group],
            label=(
                f"three-way-rotating-reconstruction-{round_index}-"
                f"{perturbed_group}-replacement"
            ),
        )
        waiters[replacement_label] = replacement
        result_queues[replacement_label] = replacement_results
        group_labels[perturbed_group].append(replacement_label)

        assert len(waiters) == 6
        assert len(group_labels[perturbed_group]) == group_sizes[perturbed_group]
        assert all(process.is_alive() for process in waiters.values())
        assert holder.is_alive()
        _assert_committed_pair(database, long_lived_reader, current_expected)
        _assert_committed_pair(database, fresh_reader, current_expected)
        ...
        for label, process in waiters.items():
            _join_cleanly(
                process,
                f"three-way rotating reconstruction waiter round {round_index} {label}",
            )
...
def test_three_way_rotating_reconstruction_gate_does_not_expand_truth_claims() -> None:
