...
def test_three_conflicting_duplicate_groups_converge_once_after_holder_rollback(
    tmp_path: Path,
) -> None:
    database = tmp_path / "three-way-duplicate-groups.sqlite3"
    ...
    holder = context.Process(
        ...
        name="three-way-duplicate-holder",
    )
    holder.start()
    ...
    mutation_ids = {
        group: f"mutation-{pending_counter}-{group}" for group in ("a", "b", "c")
    }
    values = {
        group: f"committed-{pending_counter}-{group}" for group in ("a", "b", "c")
    }

    waiters: dict[str, mp.Process] = {}
    result_queues = {}
    group_labels: dict[str, list[str]] = {"a": [], "b": [], "c": []}
    ...
    for label, process in waiters.items():
        _join_cleanly(process, f"three-way duplicate waiter {label}")
    ...
    assert names.count("generation_reuse_rejected") == 4

    winner_label = next(
        label for label, outcome in outcomes.items() if outcome["outcome"] == "applied"
    )
    winning_group = winner_label.split("_", 1)[0]
    losing_groups = [group for group in ("a", "b", "c") if group != winning_group]
...
def test_three_way_duplicate_group_gate_does_not_expand_truth_claims() -> None:
