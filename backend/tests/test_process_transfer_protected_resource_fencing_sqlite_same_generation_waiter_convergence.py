...
def _assert_successful_same_generation_convergence(
    first: dict[str, object], second: dict[str, object], *, expected_version: int
) -> None:
    outcomes = [first, second]
    assert all(outcome["kind"] == "outcome" for outcome in outcomes)
    assert [outcome["outcome"] for outcome in outcomes].count("applied") == 1
    assert [outcome["outcome"] for outcome in outcomes].count("idempotent") == 1

    for outcome in outcomes:
        assert outcome["accepted"] is True
        assert outcome["fencing_version"] == expected_version
        assert outcome["resource_version"] == expected_version
    ...
    applied = next(outcome for outcome in outcomes if outcome["outcome"] == "applied")
    replay = next(outcome for outcome in outcomes if outcome["outcome"] == "idempotent")
    assert applied["state_changed"] is True
    assert replay["state_changed"] is False


def test_same_generation_replacement_waiters_converge_after_final_holder_loss(
    tmp_path: Path,
) -> None:
    database = tmp_path / "same-generation-waiter-convergence.sqlite3"
    ...
    pending_mutation_id = f"mutation-{pending_counter}-waiter-convergence"
    pending_value = f"committed-{pending_counter}-waiter-convergence"
    converged_outcomes: tuple[dict[str, object], dict[str, object]] | None = None

    # E76 evidence gate: preserve the E75 asymmetric contender-loss/replacement path. On the
    # final controlled reconstruction, add a second independently reconstructed surviving
    # same-generation waiter. After staged-holder rollback, exactly one waiter may change
    # state; the other must resolve as the exact replay without advancing another generation.
    for reconstruction_index in range(1, RECONSTRUCTION_CYCLES + 1):
        ...
        holder = context.Process(
            ...
            name=f"waiter-convergence-holder-{reconstruction_index}",
        )
        holder.start()
        ...
        short_outcome = _run_asymmetric_contender_fates(
            ...
            label=f"waiter-convergence-{reconstruction_index}",
        )
        ...
        replacement, replacement_results = _start_replacement_waiter(
            ...
            pending_mutation_id=pending_mutation_id,
            pending_value=pending_value,
            label=f"waiter-convergence-primary-{reconstruction_index}",
        )
        ...
        peer, peer_results = _start_replacement_waiter(
            context,
            database,
            pending_counter=pending_counter,
            pending_mutation_id=pending_mutation_id,
            pending_value=pending_value,
            label=f"waiter-convergence-peer-{reconstruction_index}",
        )
        assert replacement.is_alive()
        ...
        # Final controlled holder loss releases two independently reconstructed production
        # writers for the exact same mutation. This demonstrates bounded local convergence;
        # it is not a distributed exactly-once, scheduler-fairness, or general-liveness claim.
        _terminate_holder(holder)
        first_outcome = replacement_results.get(timeout=10.0)
        second_outcome = peer_results.get(timeout=10.0)
        _join_cleanly(replacement, "primary same-generation replacement waiter")
        _join_cleanly(peer, "peer same-generation replacement waiter")
        _assert_successful_same_generation_convergence(
            first_outcome,
            second_outcome,
            expected_version=pending_version,
        )
        converged_outcomes = (first_outcome, second_outcome)

    assert converged_outcomes is not None
...
def test_same_generation_waiter_convergence_gate_does_not_expand_truth_claims() -> None:
