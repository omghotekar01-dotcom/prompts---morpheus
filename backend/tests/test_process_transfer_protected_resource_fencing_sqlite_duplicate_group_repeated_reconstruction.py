...
def test_repeated_duplicate_group_reconstruction_churn_converges_once(tmp_path: Path) -> None:
    database = tmp_path / "duplicate-group-repeated-reconstruction.sqlite3"
    ...
    holder = context.Process(
        ...
        name="duplicate-group-repeated-reconstruction-holder",
    )
    holder.start()
    ...
    for label, (mutation_id, value) in original_specs.items():
        process, results = _start_waiter(
            ...
            value=value,
            label=f"repeated-original-{label}",
        )
    ...
    _terminate_waiter(original_waiters["a_lost"], "first lost writer from mutation A")
    _terminate_waiter(original_waiters["b_lost"], "first lost writer from mutation B")
    ...
    for group, mutation_id, value in (
        ...
        process, _ = _start_waiter(
            context,
            database,
            counter=pending_counter,
            mutation_id=mutation_id,
            value=value,
            label=f"repeated-first-replacement-{group}",
        )
    ...
    # E82 evidence gate: lose the first reconstruction wave too, then reconstruct both
    # logical groups again against the exact same still-uncommitted pending generation.
    _terminate_waiter(first_wave["a"], "first replacement wave from mutation A")
    _terminate_waiter(first_wave["b"], "first replacement wave from mutation B")
    ...
    for group, mutation_id, value in (
        ...
        process, results = _start_waiter(
            ...
            mutation_id=mutation_id,
            value=value,
            label=f"repeated-second-replacement-{group}",
        )
        second_wave[group] = process
        second_results[group] = results

    live_waiters = {
        "a_survivor": original_waiters["a_survivor"],
        "a_replacement": second_wave["a"],
        "b_survivor": original_waiters["b_survivor"],
        "b_replacement": second_wave["b"],
    }
    live_results = {
        "a_survivor": original_results["a_survivor"],
        "a_replacement": second_results["a"],
        "b_survivor": original_results["b_survivor"],
        "b_replacement": second_results["b"],
    }
    ...
    for label, process in live_waiters.items():
        _join_cleanly(process, f"repeated reconstruction live waiter {label}")
...
def test_repeated_duplicate_group_reconstruction_gate_does_not_expand_truth_claims() -> None:
