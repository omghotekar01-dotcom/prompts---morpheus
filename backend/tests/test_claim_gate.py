...
def test_speedup_claim_is_blocked_until_all_measurement_roles_exist() -> None:
    incomplete = evaluate_claim("measured_speedup", ["experiment_manifest", "raw_measurements", "machine_profile"])
    ...
    complete = evaluate_claim("measured_speedup", ["experiment_manifest", "raw_measurements", "statistical_summary", "machine_profile", "baseline_manifest"])
...
def test_distribution_calibration_claim_requires_manifest_raw_and_machine() -> None:
    ...
    complete = evaluate_claim("distribution_calibration_evidence", ["distribution_calibration_manifest", "raw_measurements", "machine_profile"])
    assert complete.allowed is True
...
def test_distribution_calibration_decision_quality_requires_heldout_evaluation() -> None:
    decision = evaluate_claim("distribution_calibration_improves_decisions", ["experiment_manifest", "distribution_calibration_manifest", "raw_measurements", "machine_profile"])
...
def test_generated_migration_transition_cost_claim_requires_complete_evidence_chain() -> None:
    roles_without_attestation = ["experiment_manifest", "generated_migration_campaign", "generated_migration_campaign_summary", "machine_profile"]
    ...
    complete = evaluate_claim("generated_migration_transition_cost_measured", [*roles_without_attestation, "generated_migration_transition_cost_evidence"])
    assert complete.allowed is True
...
def test_release_bundle_fails_closed_if_any_claim_lacks_evidence() -> None:
    bundle = evaluate_claim_bundle([("generated_cpp20", ["generated_header"]), ("artifact_compiles", [])])
