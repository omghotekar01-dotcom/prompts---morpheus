...
def _start_blocked_waiter(
    ...
    *,
    ...
    waiter.start()
    assert ready.get(timeout=10.0) == name
    release.set()
    assert waiter.is_alive()
    return waiter


def test_replacement_long_waiter_loss_preserves_exact_pending_generation(
    tmp_path: Path,
) -> None:
    database = tmp_path / "replacement-long-waiter-loss-reconstruction.sqlite3"
    ...
    pending_mutation_id = f"mutation-{pending_counter}-replacement-long-waiter-loss"
    pending_value = f"committed-{pending_counter}-replacement-long-waiter-loss"
    ...
    holder = context.Process(
        ...
        name="replacement-long-waiter-staged-holder",
    )
    holder.start()
    ...
    # Heterogeneous short timeouts are deterministic contention inputs only. They establish
    # fail-closed behavior for these bounded attempts, not fairness, scheduling, or latency.
    ...
    # Lose an original blocked non-owner while the same staged holder remains alive.
    original_waiter = _start_blocked_waiter(
        context,
        database,
        name="original-long-waiter-to-lose",
    ...
    # Reconstruct another production waiter against the still-live holder and lose that
    # replacement too. Neither blocked non-owner owns the staged transaction, so neither loss
    # may consume, advance, or expose the pending generation.
    replacement_waiter = _start_blocked_waiter(
        context,
        database,
        name="replacement-long-waiter-to-lose",
        pending_counter=pending_counter,
        pending_mutation_id=pending_mutation_id,
        pending_value=pending_value,
    )
    ...
    # These forced losses occur at a deterministic blocked boundary. They are bounded fault
    # injection, not evidence of arbitrary instruction-boundary process-kill safety.
...
def test_replacement_long_waiter_loss_gate_does_not_expand_truth_claims() -> None:
