...
def test_replacement_long_waiter_losses_then_recovery_writer_loss_preserve_pending_generation(
    tmp_path: Path,
) -> None:
    database = tmp_path / "replacement-long-waiter-recovery-writer-loss.sqlite3"
    ...
    pending_mutation_id = f"mutation-{pending_counter}-recovery-writer-loss"
    pending_value = f"committed-{pending_counter}-recovery-writer-loss"
    ...
    holder = context.Process(
        ...
        name="replacement-long-waiter-recovery-loss-holder",
    )
    holder.start()
    ...
    # Distinct short busy-timeout budgets are bounded contention inputs only. Their failure
    # establishes fail-closed behavior for these attempts, not scheduler fairness or latency.
    ...
    original_waiter = _start_blocked_waiter(
        context,
        database,
        name="original-long-waiter-before-recovery-loss",
    ...
    replacement_waiter = _start_blocked_waiter(
        context,
        database,
        name="replacement-long-waiter-before-recovery-loss",
        pending_counter=pending_counter,
        pending_mutation_id=pending_mutation_id,
        pending_value=pending_value,
    )
    ...
    # Roll back the original staged owner only after both blocked non-owners are lost.
    _terminate_holder(holder)
    _assert_write_lock_released(database)
    _assert_committed_pair(database, long_lived_reader, current_expected)

    # Reconstruct that exact pending generation into a fresh transaction, then deliberately
    # terminate the recovery process after both rows are staged but before COMMIT. This known
    # deterministic boundary exercises SQLite rollback-on-process-loss only; it is not evidence
    # that arbitrary production-adapter instruction boundaries are universally kill-safe.
    ...
    recovery = context.Process(
        ...
        name="replacement-long-waiter-recovery-writer-to-lose",
    )
    recovery.start()
    ...
    # Neither pair of blocked waiter losses nor the staged recovery-writer loss may consume the
    # pending generation. A fresh production writer must still commit that exact generation once.
...
def test_replacement_long_waiter_recovery_writer_loss_gate_does_not_expand_truth_claims() -> None:
