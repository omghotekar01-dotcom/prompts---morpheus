...
from app.startup_readiness_path_extension_chain_comparison_chain_extension_chain_extension_chain_extension_evidence import (
    build_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain_extension,
...
def _e132_chains() -> tuple[dict[str, object], dict[str, object], dict[str, object]]:
    ...
    divergent = build_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain([second, third])
    return chain2, chain3, divergent


def test_e132_prefix_comparison_replays_deterministically() -> None:
    base, extended, _ = _e132_chains()
    first = build_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain_extension(base, extended)
    second = build_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain_extension(base, extended)
    assert first == second
    ...
    assert first["base_comparison_extension_chain_extension_chain_sha256"] == base["comparison_extension_chain_extension_chain_sha256"]
    assert first["candidate_comparison_extension_chain_extension_chain_sha256"] == extended["comparison_extension_chain_extension_chain_sha256"]
    assert verify_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain_extension(first) == first


def test_e132_prefix_comparison_classifies_identical_and_non_prefix() -> None:
    base, extended, divergent = _e132_chains()
    identical = build_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain_extension(base, base)
    assert identical["relation"] == "IDENTICAL"
    assert identical["extension_comparison_count"] == 0
    non_prefix = build_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain_extension(base, divergent)
    assert non_prefix["relation"] == "NOT_PREFIX_EXTENSION"
    assert non_prefix["contains_base_prefix"] is False
    reverse = build_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain_extension(extended, base)
    assert reverse["relation"] == "NOT_PREFIX_EXTENSION"


def test_e132_prefix_comparison_rejects_nested_tampering_and_semantic_forgery() -> None:
    base, extended, _ = _e132_chains()
    record = build_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain_extension(base, extended)
    tampered = deepcopy(record)
    tampered["candidate_comparison_extension_chain_extension_chain"]["comparison_extension_chain_extension_chain_sha256"] = "0" * 64
    tampered = _readdress_comparison(tampered)
    with pytest.raises(ValueError, match="comparison-prefix-chain extension chain digest does not match canonical record"):
        verify_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain_extension(tampered)
    ...
    with pytest.raises(ValueError, match="is inconsistent with embedded chains"):
        verify_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain_extension(forged)


def test_e132_prefix_comparison_rejects_authority_boundaries_and_malformed_suffix() -> None:
    base, extended, _ = _e132_chains()
    record = build_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain_extension(base, extended)
    ...
    with pytest.raises(ValueError, match="cannot carry activation authority"):
        verify_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain_extension(authority)
    ...
    with pytest.raises(ValueError, match="truth boundaries are missing"):
        verify_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain_extension(missing)
    ...
    with pytest.raises(ValueError, match="extension_comparison_sha256s is inconsistent with embedded chains"):
        verify_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain_extension(malformed)
