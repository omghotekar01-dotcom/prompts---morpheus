...
def test_three_conflicting_groups_rotate_unequal_cardinality_across_generations(
    tmp_path: Path,
) -> None:
    database = tmp_path / "three-way-rotating-cardinality.sqlite3"
    ...
    for round_index, group_sizes in enumerate(CARDINALITY_ROTATION, start=1):
        ...
        holder = context.Process(
            ...
            name=f"three-way-rotating-cardinality-holder-{round_index}",
        )
        holder.start()
        ...
        for group, group_size in group_sizes.items():
            for index in range(1, group_size + 1):
                ...
                process, results = _start_waiter(
                    ...
                    value=values[group],
                    label=f"three-way-rotating-{round_index}-{group}-{index}",
                )
        ...
        for label, process in waiters.items():
            _join_cleanly(
                process,
                f"three-way rotating-cardinality waiter round {round_index} {label}",
            )
...
def test_three_way_rotating_cardinality_gate_does_not_expand_truth_claims() -> None:
