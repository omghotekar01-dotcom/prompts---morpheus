...
import app.startup_readiness_e169_e168_sequence_prefix_evidence as e169
...
def isolate_e168_contract(monkeypatch: pytest.MonkeyPatch) -> None:
    def verify(record: object) -> dict[str, object]:
        if not isinstance(record, dict) or record.get("reject"):
            raise ValueError("nested E168 rejected")
        return deepcopy(record)
    monkeypatch.setattr(e169, "verify_startup_readiness_e168_e167_comparison_sequence", verify)


def test_e169_builds_replays_and_classifies_relations_deterministically() -> None:
    ...
    same = e169.build_startup_readiness_e169_e168_sequence_prefix_comparison(base, identical)
    assert same["relation"] == "IDENTICAL"
    assert same["extension_comparison_sha256s"] == []
    assert same["extension_comparison_count"] == 0
    extended = e169.build_startup_readiness_e169_e168_sequence_prefix_comparison(base, strict)
    ...
    assert e169.verify_startup_readiness_e169_e168_sequence_prefix_comparison(extended) == extended
    not_prefix = e169.build_startup_readiness_e169_e168_sequence_prefix_comparison(base, divergent)
    assert not_prefix["relation"] == "NOT_PREFIX_EXTENSION"
    assert not_prefix["extension_comparison_sha256s"] == []


def test_e169_rejects_nested_malformed_and_duplicate_identities() -> None:
    base = _sequence("base", ["c1", "c2"])
    with pytest.raises(ValueError, match="nested E168 rejected"):
        e169.build_startup_readiness_e169_e168_sequence_prefix_comparison(base, {"reject": True})
    malformed_sequence = _sequence("bad", ["c1", "c2"]); malformed_sequence["sequence_sha256"] = "bad"
    with pytest.raises(ValueError, match="valid E168 sequence identities"):
        e169.build_startup_readiness_e169_e168_sequence_prefix_comparison(base, malformed_sequence)
    malformed_comparison = _sequence("bad-comparison", ["c1", "c2"]); malformed_comparison["comparison_sha256s"][1] = "bad"
    with pytest.raises(ValueError, match="valid ordered E167"):
        e169.build_startup_readiness_e169_e168_sequence_prefix_comparison(base, malformed_comparison)
    duplicate = _sequence("duplicate", ["c1", "c1"])
    with pytest.raises(ValueError, match="unique ordered E167"):
        e169.build_startup_readiness_e169_e168_sequence_prefix_comparison(base, duplicate)


def test_e169_rejects_semantic_boundary_authority_embedded_and_digest_tampering() -> None:
    original = e169.build_startup_readiness_e169_e168_sequence_prefix_comparison(
    ...
    for record in mutations:
        with pytest.raises(ValueError):
            e169.verify_startup_readiness_e169_e168_sequence_prefix_comparison(record)
