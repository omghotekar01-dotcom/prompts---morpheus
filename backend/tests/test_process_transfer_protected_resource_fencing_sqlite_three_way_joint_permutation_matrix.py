...
GROUPS = ("A", "B", "C")
CARDINALITY_PERMUTATIONS = tuple(itertools.permutations((1, 2, 3)))
...
def test_three_way_dual_reconstruction_joint_permutation_matrix(tmp_path: Path) -> None:
    assert len(JOINT_CASES) == 6
    assert {sizes for sizes, _ in JOINT_CASES} == set(itertools.permutations((1, 2, 3)))
    assert {order for _, order in JOINT_CASES} == set(itertools.permutations(GROUPS))

    context = mp.get_context("spawn")

    for case_index, (sizes, start_order) in enumerate(JOINT_CASES, start=1):
        group_sizes = dict(zip(GROUPS, sizes, strict=True))
        database = tmp_path / f"three-way-joint-permutation-{case_index}.sqlite3"
        ...
        holder = context.Process(
            ...
            name=f"three-way-joint-permutation-holder-{case_index}",
        )
        holder.start()
        ...
        for group in start_order:
            for member_index in range(1, group_sizes[group] + 1):
                ...
                process, results = _start_waiter(
                    ...
                    value=values[group],
                    label=f"three-way-joint-{case_index}-{group}-{member_index}",
                )
        ...
        for group in reconstructed_groups:
            ...
            _terminate_waiter(
                lost_process,
                f"joint permutation {case_index} blocked member from mutation {group}",
            )
            ...
            replacement, replacement_results = _start_waiter(
                ...
                mutation_id=mutation_ids[group],
                value=values[group],
                label=f"three-way-joint-{case_index}-{group}-replacement",
            )
        ...
        for label, process in waiters.items():
            _join_cleanly(process, f"three-way joint waiter {case_index} {label}")
        ...
        for group in losing_groups:
            assert [outcomes[label]["outcome"] for label in group_labels[group]] == [
...
def test_three_way_joint_permutation_gate_does_not_expand_truth_claims() -> None:
