...
GROUPS = ("A", "B", "C")
GROUP_SIZES = {"A": 1, "B": 2, "C": 3}
START_ORDERS = tuple(itertools.permutations(GROUPS))
BASE_COUNTER = 5801


def test_three_way_dual_reconstruction_is_invariant_to_logical_group_start_order(
    tmp_path: Path,
) -> None:
    assert len(START_ORDERS) == 6
    assert set(START_ORDERS) == set(itertools.permutations(GROUPS))

    context = mp.get_context("spawn")

    for order_index, start_order in enumerate(START_ORDERS, start=1):
        database = tmp_path / f"three-way-start-order-{order_index}.sqlite3"
        ...
        holder = context.Process(
            ...
            name=f"three-way-start-order-holder-{order_index}",
        )
        holder.start()
        ...
        for group in start_order:
            for member_index in range(1, GROUP_SIZES[group] + 1):
                ...
                process, results = _start_waiter(
                    ...
                    value=values[group],
                    label=f"three-way-start-order-{order_index}-{group}-{member_index}",
                )
        ...
        for group in ("B", "C"):
            ...
            _terminate_waiter(
                lost_process,
                f"start-order {order_index} blocked member from mutation {group}",
            )
            ...
            replacement, replacement_results = _start_waiter(
                ...
                mutation_id=mutation_ids[group],
                value=values[group],
                label=f"three-way-start-order-{order_index}-{group}-replacement",
            )
        ...
        for label, process in waiters.items():
            _join_cleanly(process, f"three-way start-order waiter {order_index} {label}")
...
def test_three_way_start_order_gate_does_not_expand_truth_claims() -> None:
