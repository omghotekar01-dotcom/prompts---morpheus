...
SHORT_TIMEOUT_SECONDS = SHORT_TIMEOUT_BUDGETS_SECONDS[0]


def _run_asymmetric_contender_fates(
    ...
    *,
    ...
    long_results = context.Queue()

    short_actor = f"{label}-short-timeout"
    long_actor = f"{label}-long-forced-loss"
    short_contender = context.Process(
        ...
        args=(
            ...
            short_results,
            pending_counter,
            f"{pending_mutation_id}-{short_actor}",
            f"{pending_value}-{short_actor}",
        ),
        name=short_actor,
    )
    long_contender = context.Process(
        target=_contend_for_pending_generation,
        args=(
            str(database),
            long_actor,
            ...
            long_results,
            pending_counter,
            f"{pending_mutation_id}-{long_actor}",
            f"{pending_value}-{long_actor}",
        ),
        name=long_actor,
    )

    short_contender.start()
    long_contender.start()
    ...
    # Both independently reconstructed writers are released together. The short writer must
    # complete its bounded fail-closed busy-timeout path while the deliberately longer waiter
    # remains blocked. We then force-loss that longer waiter before its deadline. These timeout
    # values and the ordering asserted here are deterministic test controls only, not scheduler,
    # fairness, liveness, or latency guarantees.
    release.set()
    short_outcome = short_results.get(timeout=3.0)
    _join_cleanly(short_contender, short_actor)

    assert short_outcome["actor"] == short_actor
    assert short_outcome["kind"] == "sqlite-failure"
    assert (
        "SQLite transactionally fenced protected-resource mutation failed"
        in str(short_outcome["message"])
    )
    assert holder.is_alive()
    assert long_contender.is_alive()

    _terminate_holder(long_contender)
    assert holder.is_alive()
    return short_outcome


def test_asymmetric_contender_loss_during_repeated_reconstruction_preserves_generation(
    tmp_path: Path,
) -> None:
    database = tmp_path / "asymmetric-reconstruction-contender-loss.sqlite3"
    ...
    pending_mutation_id = f"mutation-{pending_counter}-asymmetric-reconstruction"
    pending_value = f"committed-{pending_counter}-asymmetric-reconstruction"

    # E74 evidence gate: preserve one unchanged pending generation through repeated staged
    # reconstruction cycles where simultaneous contenders have asymmetric fates: the short
    # timeout fails closed, then the still-blocked long waiter is forcibly lost before the
    # staged holder itself is deliberately terminated at the known uncommitted boundary.
    for reconstruction_index in range(1, RECONSTRUCTION_CYCLES + 1):
        ...
        holder = context.Process(
            ...
            name=f"asymmetric-reconstruction-holder-{reconstruction_index}",
        )
        holder.start()
        ...
        # The completed short timeout and forced loss of the longer waiter must not expose or
        # consume the staged pending generation. Both reader constructions remain coherent while
        # the original staged transaction is still alive.
        _assert_committed_pair(database, long_lived_reader, current_expected)
        _assert_committed_pair(database, fresh_reader, current_expected)
        assert holder.is_alive()
...
def test_asymmetric_reconstruction_contender_loss_gate_does_not_expand_truth_claims() -> None:
