...
import app.startup_readiness_e171_e170_sequence_prefix_evidence as e171
...
def isolate_e170_contract(monkeypatch: pytest.MonkeyPatch) -> None:
    def verify(record: object) -> dict[str, object]:
        if not isinstance(record, dict) or record.get("reject"):
            raise ValueError("nested E170 rejected")
        return deepcopy(record)
    monkeypatch.setattr(e171, "verify_startup_readiness_e170_e169_comparison_sequence", verify)


def test_e171_builds_replays_and_classifies_relations_deterministically() -> None:
    ...
    same = e171.build_startup_readiness_e171_e170_sequence_prefix_comparison(base, identical)
    assert same["relation"] == "IDENTICAL"
    assert same["extension_comparison_sha256s"] == []
    assert same["extension_comparison_count"] == 0
    extended = e171.build_startup_readiness_e171_e170_sequence_prefix_comparison(base, strict)
    ...
    assert e171.verify_startup_readiness_e171_e170_sequence_prefix_comparison(extended) == extended
    not_prefix = e171.build_startup_readiness_e171_e170_sequence_prefix_comparison(base, divergent)
    assert not_prefix["relation"] == "NOT_PREFIX_EXTENSION"
    assert not_prefix["extension_comparison_sha256s"] == []


def test_e171_rejects_nested_malformed_and_duplicate_identities() -> None:
    base = _sequence("base", ["c1", "c2"])
    with pytest.raises(ValueError, match="nested E170 rejected"):
        e171.build_startup_readiness_e171_e170_sequence_prefix_comparison(base, {"reject": True})
    malformed_sequence = _sequence("bad", ["c1", "c2"]); malformed_sequence["sequence_sha256"] = "bad"
    with pytest.raises(ValueError, match="valid E170 sequence identities"):
        e171.build_startup_readiness_e171_e170_sequence_prefix_comparison(base, malformed_sequence)
    malformed_comparison = _sequence("bad-comparison", ["c1", "c2"]); malformed_comparison["comparison_sha256s"][1] = "bad"
    with pytest.raises(ValueError, match="valid ordered E169"):
        e171.build_startup_readiness_e171_e170_sequence_prefix_comparison(base, malformed_comparison)
    duplicate = _sequence("duplicate", ["c1", "c1"])
    with pytest.raises(ValueError, match="unique ordered E169"):
        e171.build_startup_readiness_e171_e170_sequence_prefix_comparison(base, duplicate)


def test_e171_rejects_semantic_boundary_authority_embedded_and_digest_tampering() -> None:
    original = e171.build_startup_readiness_e171_e170_sequence_prefix_comparison(
    ...
    for record in mutations:
        with pytest.raises(ValueError):
            e171.verify_startup_readiness_e171_e170_sequence_prefix_comparison(record)
