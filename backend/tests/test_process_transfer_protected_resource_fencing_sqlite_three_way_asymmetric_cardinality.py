...
def test_three_conflicting_groups_with_unequal_cardinality_converge_once_after_holder_rollback(
    tmp_path: Path,
) -> None:
    database = tmp_path / "three-way-asymmetric-cardinality.sqlite3"
    ...
    holder = context.Process(
        ...
        name="three-way-asymmetric-cardinality-holder",
    )
    holder.start()
    ...
    for group, group_size in GROUP_SIZES.items():
        for index in range(1, group_size + 1):
            ...
            process, results = _start_waiter(
                ...
                value=values[group],
                label=f"three-way-asymmetric-{group}-{index}",
            )
    ...
    for label, process in waiters.items():
        _join_cleanly(process, f"three-way asymmetric-cardinality waiter {label}")
...
def test_three_way_asymmetric_cardinality_gate_does_not_expand_truth_claims() -> None:
