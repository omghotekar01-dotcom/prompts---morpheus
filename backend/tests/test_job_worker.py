...
def _worker(*, timeout: float = 10.0, output: int = 4096) -> LocalBoundedJobWorker:
    # Windows hosted runners can take several seconds to start the first Python
    # child process after MSVC environment setup. Functional worker tests use a
    # generous deadline; the dedicated timeout test below still proves the
    # timeout path with an explicit 150 ms budget.
...
def test_worker_executes_allowlisted_process_without_shell_and_cleans_workspace() -> None:
    ...
    try:
        ...
        result = worker.wait(submitted["job_id"], timeout_seconds=20)
...
def test_worker_times_out_long_running_process() -> None:
    ...
    try:
        ...
        result = worker.wait(submitted["job_id"], timeout_seconds=5)
