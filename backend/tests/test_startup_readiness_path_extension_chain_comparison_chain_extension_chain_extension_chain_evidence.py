...
from app.startup_readiness_path_extension_chain_comparison_chain_extension_chain_extension_evidence import (
    build_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension,
)
from app.startup_readiness_path_extension_chain_comparison_chain_extension_chain_extension_chain_evidence import (
    build_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain,
...
def _readdress_chain(record: dict[str, object]) -> dict[str, object]:
    core = {
        ...
        if key != "comparison_extension_chain_extension_chain_sha256"
    }
...
def _e131_records() -> tuple[dict[str, object], dict[str, object], dict[str, object]]:
    ...
    transitions = [build_startup_readiness_coherence_transition(reports[index], reports[index + 1]) for index in range(3)]
    ...
    strict = build_startup_readiness_coherence_path_extension_chain_comparison_chain_extension(comparison_chain2, comparison_chain3)
    identical = build_startup_readiness_coherence_path_extension_chain_comparison_chain_extension(comparison_chain3, comparison_chain3)
    reverse = build_startup_readiness_coherence_path_extension_chain_comparison_chain_extension(comparison_chain3, comparison_chain2)
...
def test_comparison_prefix_chain_extension_chain_replays_deterministically() -> None:
    first, second, third = _e131_records()
    chain = build_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain(
        [first, second, third]
    )
    replay = build_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain(
        [first, second, third]
    )
    ...
    assert chain["start_comparison_extension_chain_sha256"] == first["base_comparison_extension_chain_sha256"]
    assert chain["end_comparison_extension_chain_sha256"] == third["candidate_comparison_extension_chain_sha256"]
    ...
    assert verify_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain(chain) == chain


def test_comparison_prefix_chain_extension_chain_rejects_minimum_duplicate_and_broken_adjacency() -> None:
    first, second, _ = _e131_records()
    with pytest.raises(ValueError, match="at least two comparison records"):
        build_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain([first])

    with pytest.raises(ValueError, match="duplicate comparison identities"):
        build_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain([first, first])

    with pytest.raises(ValueError, match="broken structural adjacency"):
        build_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain([second, first])


def test_comparison_prefix_chain_extension_chain_rejects_nested_tampering_and_summary_forgery() -> None:
    first, second, third = _e131_records()
    chain = build_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain(
        [first, second, third]
    )
    ...
    with pytest.raises(ValueError, match="comparison-prefix-chain extension digest does not match canonical record"):
        verify_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain(tampered)
    ...
    with pytest.raises(ValueError, match="relation_counts is inconsistent with embedded comparisons"):
        verify_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain(forged)


def test_comparison_prefix_chain_extension_chain_rejects_authority_boundaries_and_malformed_identity() -> None:
    first, second, third = _e131_records()
    chain = build_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain(
        [first, second, third]
    )
    ...
    with pytest.raises(ValueError, match="cannot carry activation authority"):
        verify_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain(authority)
    ...
    with pytest.raises(ValueError, match="truth boundaries are missing"):
        verify_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain(missing)
    ...
    with pytest.raises(ValueError, match="comparison_sha256s is inconsistent with embedded comparisons"):
        verify_startup_readiness_coherence_path_extension_chain_comparison_chain_extension_chain_extension_chain(malformed)
