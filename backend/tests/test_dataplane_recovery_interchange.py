...
from app.dataplane_recovery_interchange import (
    EVIDENCE_STATE,
    INTERCHANGE_SCHEMA,
    TRUTH_BOUNDARY,
    export_recovery_checkpoint,
    import_recovery_checkpoint,
)
...
def test_p59_interchange_is_deterministic_and_exactly_roundtrips() -> None:
    ...
    assert payload_a == json.dumps(json.loads(payload_a), sort_keys=True, separators=(",", ":"), ensure_ascii=False).encode("utf-8")
