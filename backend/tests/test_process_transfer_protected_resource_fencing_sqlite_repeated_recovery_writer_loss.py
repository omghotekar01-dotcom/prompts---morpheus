...
def test_repeated_recovery_writer_losses_preserve_one_pending_generation(
    ...
    # Reconstruct and lose several independent recovery transactions at the same
    # deterministic staged-but-uncommitted boundary.  Every loss must roll back to
    # the same previously committed pair and release SQLite's write lock.  This is
    # bounded fault-injection evidence only; it is not a claim about arbitrary
    # instruction-boundary termination in the production adapter.
    for loss_index in range(1, RECOVERY_LOSSES + 1):
        ...
        recovery = context.Process(
            ...
            name=f"recovery-loss-{loss_index}",
        )
        recovery.start()
        ...
        # Uncommitted staged rows must never become visible to either an existing
        # reader connection or a reader reconstructed after this loss actor began.
        ...
        # Each injected loss leaves the exact same pending generation available;
        # no counter/version is consumed and no partial row pair leaks.
        assert _pair_payload(
            long_lived_reader.snapshot_pair(RESOURCE_ID, AUTHORITY_ID)
        ) == expected
        assert _fresh_pair(database) == expected
