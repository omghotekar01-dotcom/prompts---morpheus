...
def _assert_complete_derivation_invariants(records: tuple[Record, ...]) -> None:
    assert len(records) == manifest_gate.EXPECTED_CASE_COUNT == 36

    namespaces = [derived[0] for _, derived in records]
    assert len(namespaces) == len(set(namespaces)) == len(records)

    seen_literals: set[str] = set()
    for _, (_, literals) in records:
...
def _verify_shard_manifest(manifest: bytes, payload: bytes) -> tuple[Record, ...]:
    ...
    if (
        not isinstance(raw["logical_case_count"], int)
        or isinstance(raw["logical_case_count"], bool)
        or raw["logical_case_count"] <= 0
    ):
...
def test_manifested_shards_merge_to_single_shot_payload_and_manifest() -> None:
    ...
    _assert_complete_derivation_invariants(expected)

    for layout in layouts:
        merged = _round_trip_manifested_layout(layout)
        assert merged == expected
        _assert_complete_derivation_invariants(merged)
