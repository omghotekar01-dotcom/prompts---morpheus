...
def test_repeated_fresh_reconstruction_contention_and_loss_preserves_same_generation(
    ...
    holder = context.Process(
        ...
        name="repeated-fresh-loss-original-holder",
    )
    holder.start()
    ...
    original_waiter = _start_blocked_waiter(
        context,
        database,
        name="original-long-waiter-before-repeated-fresh-loss",
        pending_counter=pending_counter,
        pending_mutation_id=pending_mutation_id,
        pending_value=pending_value,
    )
    ...
    replacement_waiter = _start_blocked_waiter(
        context,
        database,
        name="replacement-long-waiter-before-repeated-fresh-loss",
        pending_counter=pending_counter,
        pending_mutation_id=pending_mutation_id,
        pending_value=pending_value,
    )
    ...
    # Reconstruct the same pending generation through the already-bounded repeated recovery path.
    # On the final cycle, lose the longer-timeout waiter while its staged holder remains alive.
    for recovery_index in range(1, RECOVERY_CYCLES + 1):
        ...
        recovery = context.Process(
            ...
            name=f"repeated-fresh-loss-recovery-holder-{recovery_index}",
        )
        recovery.start()
        ...
        if recovery_index == RECOVERY_CYCLES:
            final_waiter = _start_blocked_waiter(
                context,
                database,
                name="final-long-waiter-before-repeated-fresh-loss",
                pending_counter=pending_counter,
                pending_mutation_id=pending_mutation_id,
                pending_value=pending_value,
            )
    ...
    for reconstruction_index in range(1, FRESH_RECONSTRUCTION_LOSSES + 1):
        ...
        reconstruction = context.Process(
            ...
            name=f"fresh-reconstruction-loss-{reconstruction_index}",
        )
        reconstruction.start()
        assert reconstruction_ready.get(timeout=10.0) == {
            "counter": pending_counter,
            "version": pending_version,
        }
        ...
        _terminate_holder(reconstruction)
        _assert_write_lock_released(database)
        _assert_committed_pair(database, long_lived_reader, current_expected)
        _assert_committed_pair(database, fresh_reader, current_expected)
