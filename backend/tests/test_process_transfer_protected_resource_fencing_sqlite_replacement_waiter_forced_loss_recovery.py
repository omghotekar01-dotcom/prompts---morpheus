...
def test_replacement_waiter_forced_loss_preserves_pending_generation_and_recovers_once(
    tmp_path: Path,
) -> None:
    database = tmp_path / "replacement-waiter-forced-loss-recovery.sqlite3"
    ...
    for round_index, release_order in enumerate(RELEASE_ORDERS, start=1):
        ...
        pending_mutation_id = f"mutation-{pending_counter}-double-loss-{round_index}"
        pending_value = f"committed-{pending_counter}-double-loss-{round_index}"
        ...
        _join_cleanly(short, "short double-loss waiter")
        ...
        # Lose the reconstructed replacement while it is still a non-owning blocked waiter.
        # This second waiter loss must not consume, expose, or partially advance the staged
        # generation while the original holder remains alive.
        _terminate_blocked_waiter(replacement)
        assert holder.is_alive()
        assert _pair_payload(long_lived_reader.snapshot_pair(RESOURCE_ID, AUTHORITY_ID)) == expected
        assert _fresh_pair(database) == expected

        # Only after both blocked waiters have been lost do we remove the uncommitted holder.
        # A newly reconstructed normal writer must still be able to commit exactly the same
        # pending generation once, demonstrating containment rather than generation skipping.
...
def test_replacement_waiter_forced_loss_gate_does_not_expand_truth_claims() -> None:
