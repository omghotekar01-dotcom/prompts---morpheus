...
import app.startup_readiness_e199_e198_sequence_prefix_evidence as e199
...
def isolate_e198_contract(monkeypatch: pytest.MonkeyPatch) -> None:
    def verify(record: object) -> dict[str, object]:
        if not isinstance(record, dict) or record.get("reject"):
            raise ValueError("nested E198 rejected")
        return deepcopy(record)
    monkeypatch.setattr(e199, "verify_startup_readiness_e198_e197_comparison_sequence", verify)


def test_e199_classifies_identical_strict_prefix_and_not_prefix() -> None:
    ...
    same = e199.build_startup_readiness_e199_e198_sequence_prefix_comparison(base, identical)
    assert same["relation"] == "IDENTICAL"
    assert same["extension_comparison_sha256s"] == []
    extension = e199.build_startup_readiness_e199_e198_sequence_prefix_comparison(base, strict)
    ...
    other = e199.build_startup_readiness_e199_e198_sequence_prefix_comparison(base, divergent)
    assert other["relation"] == "NOT_PREFIX_EXTENSION"
    assert other["extension_comparison_sha256s"] == []
    assert e199.verify_startup_readiness_e199_e198_sequence_prefix_comparison(extension) == extension


def test_e199_rejects_nested_identity_and_duplicate_failures() -> None:
    base = _sequence("base", ["p1", "p2"])
    with pytest.raises(ValueError, match="nested E198 rejected"):
        e199.build_startup_readiness_e199_e198_sequence_prefix_comparison(base, {"reject": True})
    malformed_sequence = _sequence("candidate", ["p1", "p2"]); malformed_sequence["sequence_sha256"] = "bad"
    with pytest.raises(ValueError, match="valid E198"):
        e199.build_startup_readiness_e199_e198_sequence_prefix_comparison(base, malformed_sequence)
    malformed_ordered = _sequence("candidate", ["p1", "p2"]); malformed_ordered["comparison_sha256s"][1] = "bad"
    with pytest.raises(ValueError, match="valid ordered E197"):
        e199.build_startup_readiness_e199_e198_sequence_prefix_comparison(base, malformed_ordered)
    duplicate = _sequence("candidate", ["p1", "p1"])
    with pytest.raises(ValueError, match="unique ordered E197"):
        e199.build_startup_readiness_e199_e198_sequence_prefix_comparison(base, duplicate)


def test_e199_rejects_relation_suffix_count_boundary_authority_embedded_and_digest_tampering() -> None:
    original = e199.build_startup_readiness_e199_e198_sequence_prefix_comparison(
    ...
    for record in mutations:
        with pytest.raises(ValueError):
            e199.verify_startup_readiness_e199_e198_sequence_prefix_comparison(record)
