...
def _start_waiter(
    ...
    *,
    ...
    return _start_replacement_waiter(
        context,
        database,
...
def test_asymmetric_duplicate_group_reconstruction_history_converges_once(
    tmp_path: Path,
) -> None:
    database = tmp_path / "duplicate-group-asymmetric-reconstruction.sqlite3"
    ...
    holder = context.Process(
        ...
        name="duplicate-group-asymmetric-reconstruction-holder",
    )
    holder.start()
    ...
    for label, (mutation_id, value) in original_specs.items():
        process, results = _start_waiter(
            ...
            value=value,
            label=f"asymmetric-original-{label}",
        )
    ...
    _terminate_waiter(original_waiters["a_lost"], "lost writer from mutation A")
    _terminate_waiter(original_waiters["b_lost"], "lost writer from mutation B")

    first_wave = {}
    first_results = {}
    for group, mutation_id, value in (
        ("a", mutation_a_id, mutation_a_value),
        ("b", mutation_b_id, mutation_b_value),
    ):
        process, results = _start_waiter(
            context,
            database,
            ...
            mutation_id=mutation_id,
            value=value,
            label=f"asymmetric-first-replacement-{group}",
        )
        first_wave[group] = process
        first_results[group] = results

    assert all(process.is_alive() for process in first_wave.values())
    ...
    # E83 evidence gate: only mutation A receives another loss/reconstruction cycle.
    # Mutation B keeps its first reconstructed waiter alive, giving the two logical
    # groups intentionally asymmetric reconstruction histories at the same generation.
    _terminate_waiter(first_wave["a"], "first replacement wave from mutation A")
    second_a, second_a_results = _start_waiter(
        context,
        database,
        counter=pending_counter,
        mutation_id=mutation_a_id,
        value=mutation_a_value,
        label="asymmetric-second-replacement-a",
    )

    live_waiters = {
        "a_survivor": original_waiters["a_survivor"],
        "a_replacement": second_a,
        "b_survivor": original_waiters["b_survivor"],
        "b_replacement": first_wave["b"],
    }
    live_results = {
        "a_survivor": original_results["a_survivor"],
        "a_replacement": second_a_results,
        "b_survivor": original_results["b_survivor"],
        "b_replacement": first_results["b"],
    }
    ...
    for label, process in live_waiters.items():
        _join_cleanly(process, f"asymmetric reconstruction live waiter {label}")
    ...
    a_names = [outcomes["a_survivor"]["outcome"], outcomes["a_replacement"]["outcome"]]
    b_names = [outcomes["b_survivor"]["outcome"], outcomes["b_replacement"]["outcome"]]
    ...
    if winning_group == "a":
        winning_mutation_id, winning_value = mutation_a_id, mutation_a_value
        losing_mutation_id, losing_value = mutation_b_id, mutation_b_value
    else:
        winning_mutation_id, winning_value = mutation_b_id, mutation_b_value
        losing_mutation_id, losing_value = mutation_a_id, mutation_a_value
    ...
    replay_writer = SQLiteTransactionallyFencedProtectedResource(database, timeout_seconds=3.0)
...
def test_asymmetric_duplicate_group_reconstruction_gate_does_not_expand_truth_claims() -> None:
