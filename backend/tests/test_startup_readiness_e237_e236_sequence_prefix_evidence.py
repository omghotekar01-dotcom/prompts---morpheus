...
import app.startup_readiness_e237_e236_sequence_prefix_evidence as e237
...
def isolate_e236_contract(monkeypatch: pytest.MonkeyPatch) -> None:
    def verify(record: object) -> dict[str, object]:
        if not isinstance(record, dict) or record.get("reject"):
            raise ValueError("nested E236 rejected")
...
def test_e237_classifies_identical_strict_prefix_and_non_prefix() -> None:
    base = _sequence("s1", ["c1", "c2"])
    identical = _sequence("s2", ["c1", "c2"])
    extended = _sequence("s3", ["c1", "c2", "c3"])
    divergent = _sequence("s4", ["c1", "other"])

    same = e237.build_startup_readiness_e237_e236_sequence_prefix_comparison(base, identical)
    assert same["relation"] == "IDENTICAL"
    assert same["extension_comparison_sha256s"] == []
    assert same["extension_comparison_count"] == 0

    strict = e237.build_startup_readiness_e237_e236_sequence_prefix_comparison(base, extended)
    ...
    other = e237.build_startup_readiness_e237_e236_sequence_prefix_comparison(base, divergent)
    ...
    assert e237.verify_startup_readiness_e237_e236_sequence_prefix_comparison(strict) == strict


def test_e237_rejects_nested_malformed_and_duplicate_identities() -> None:
    base = _sequence("s1", ["c1", "c2"])
    with pytest.raises(ValueError, match="nested E236 rejected"):
        e237.build_startup_readiness_e237_e236_sequence_prefix_comparison(base, {"reject": True})

    malformed_sequence = _sequence("s2", ["c1", "c2"])
    malformed_sequence["sequence_sha256"] = "bad"
    with pytest.raises(ValueError, match="valid E236"):
        e237.build_startup_readiness_e237_e236_sequence_prefix_comparison(base, malformed_sequence)

    malformed_ordered = _sequence("s2", ["c1", "c2"])
    malformed_ordered["comparison_sha256s"] = ["bad", _digest("c2")]
    with pytest.raises(ValueError, match="valid ordered E235"):
        e237.build_startup_readiness_e237_e236_sequence_prefix_comparison(base, malformed_ordered)

    duplicate = _sequence("s2", ["c1", "c1"])
    with pytest.raises(ValueError, match="unique ordered E235"):
        e237.build_startup_readiness_e237_e236_sequence_prefix_comparison(base, duplicate)


def test_e237_rejects_relation_suffix_count_boundary_authority_embedded_and_digest_tampering() -> None:
    base = _sequence("s1", ["c1", "c2"])
    candidate = _sequence("s2", ["c1", "c2", "c3"])
    original = e237.build_startup_readiness_e237_e236_sequence_prefix_comparison(base, candidate)
    ...
    for record in mutations:
        with pytest.raises(ValueError):
            e237.verify_startup_readiness_e237_e236_sequence_prefix_comparison(record)
