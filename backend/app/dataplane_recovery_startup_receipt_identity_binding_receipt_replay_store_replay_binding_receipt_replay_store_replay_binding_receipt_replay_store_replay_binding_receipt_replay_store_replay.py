"""Replay-verify the locally retained P96 P95-replay identity record.

P97 independently reopens one selected P96 record and verifies exact byte
identity, strict canonical JSON, exact schema, the embedded P95 evidence-state
identity, and semantic agreement with caller-supplied P96 evidence.
...
from .dataplane_recovery_startup_receipt_identity_binding_receipt_replay_store_replay_binding_receipt_replay_store_replay_binding_receipt_replay_store_replay_binding_receipt_replay_store import (
...
EVIDENCE_STATE = (
    ...
    "VERIFIED_IDENTITY_STORED_REPLAY_VERIFIED"
)
TRUTH_BOUNDARY = (
    "This historical read-only gate proves only that one selected local P96 record matched the caller-supplied P96 evidence during "
    "this call: exact stored byte length and SHA-256, strict canonical JSON, exact schema, embedded P95 evidence-state identity, and "
    "semantic fields were verified. It does not authenticate P96 evidence or the filesystem, prove freshness/latest/global/monotonic "
    "head truth, prevent replay/rollback or coordinated replacement, provide an atomic snapshot after return, rerun P95/P96 or their "
...
_REQUIRED_P96_FLAGS = (
    "p95_evidence_state_verified",
    "p95_verification_flags_verified",
    "exact_readback_verified",
)
_SCHEMA_KEYS = frozenset({field for field, _ in _FIELDS} | {"p95_evidence_state"})
...
class RecoveryStartupReplayRetainedReceiptIdentityBindingReceiptReplayIdentityStoreReplayEvidence:
    sequence: int
    lineage_sha256: str
    ...
    source_path: str
    p96_evidence_state_verified: bool
    p96_verification_flags_verified: bool
...
def verify_recovery_startup_replay_retained_receipt_identity_binding_receipt_replay_identity_store(
    ...
    *,
    ...
    """Replay-verify one retained P96 identity record against compatible P96 evidence."""
    if not isinstance(evidence, RecoveryStartupReplayRetainedReceiptIdentityBindingReceiptReplayIdentityStoreEvidence):
        raise ValueError("P96 stored replay identity evidence has an incompatible type")
    if evidence.evidence_state != P96_EVIDENCE_STATE:
        raise ValueError("P96 stored replay identity evidence state is incompatible")
    if evidence.automatic_control_allowed is not False:
        raise ValueError("P96 stored replay identity evidence must not grant automatic-control authority")
    if any(getattr(evidence, flag, None) is not True for flag in _REQUIRED_P96_FLAGS):
        raise ValueError("P96 stored replay identity verification flags are incomplete")

    values: dict[str, object] = {}
    for field, kind in _FIELDS:
        raw = getattr(evidence, field, None)
        values[field] = (
            _positive_int(raw, field=f"P96 {field}")
            if kind == "int"
            else _sha256(raw, field=f"P96 {field}")
        )

    stored_sha = _sha256(evidence.stored_payload_sha256, field="P96 stored payload SHA-256")
    stored_size = _positive_int(evidence.stored_payload_size_bytes, field="P96 stored payload size")
    ...
    if len(raw) != stored_size:
        raise ValueError("retained P96 replay identity byte length mismatch")
    if hashlib.sha256(raw).hexdigest() != stored_sha:
        raise ValueError("retained P96 replay identity SHA-256 mismatch")

    try:
        text = raw.decode("utf-8")
    except UnicodeDecodeError as exc:
        raise ValueError("retained P96 replay identity is not valid UTF-8") from exc
    try:
        payload = json.loads(text)
    except json.JSONDecodeError as exc:
        raise ValueError("retained P96 replay identity is not valid JSON") from exc
    if not isinstance(payload, dict):
        raise ValueError("retained P96 replay identity must be a JSON object")
    if frozenset(payload) != _SCHEMA_KEYS:
        raise ValueError("retained P96 replay identity schema is incompatible")
    if _canonical(payload) != raw:
        raise ValueError("retained P96 replay identity is not strict canonical JSON")
    if payload.get("p95_evidence_state") != P95_EVIDENCE_STATE:
        raise ValueError("retained P96 replay identity embeds an incompatible P95 evidence state")

    for field, kind in _FIELDS:
        raw_value = payload.get(field)
        parsed = (
            _positive_int(raw_value, field=f"retained P96 {field}")
            if kind == "int"
            else _sha256(raw_value, field=f"retained P96 {field}")
        )
        if parsed != values[field]:
            raise ValueError(f"retained P96 replay identity disagrees on {field}")

    return RecoveryStartupReplayRetainedReceiptIdentityBindingReceiptReplayIdentityStoreReplayEvidence(
        **values,
