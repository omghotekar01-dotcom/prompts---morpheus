"""Persist the minimum verified P95 canonical P94-replay identity locally.

P96 closes only the local-retention seam left explicit by P95. A caller supplies
verified P95 replay evidence; this gate validates that evidence contract, reduces
...
EVIDENCE_STATE = (
    ...
    "RECEIPT_REPLAY_IDENTITY_STORED_REPLAY_BINDING_RECEIPT_REPLAY_VERIFIED_IDENTITY_STORED_REPLAY_BINDING_RECEIPT_REPLAY_VERIFIED_IDENTITY_STORED"
)
TRUTH_BOUNDARY = (
    "This gate proves only that compatible verified P95 replay evidence was reduced to a strict canonical local identity record, "
    ...
    "monotonic trust anchor. This gate does not authenticate P95 evidence or the filesystem, prove freshness/latest/global/monotonic "
    "head truth, prevent rollback/replay or coordinated replacement, rerun P94/P95 or their dependencies, authorize startup or mutation, "
...
class RecoveryStartupReplayRetainedReceiptIdentityBindingReceiptReplayIdentityStoreEvidence:
    sequence: int
    lineage_sha256: str
    ...
    p95_evidence_state_verified: bool
    p95_verification_flags_verified: bool
...
def store_recovery_startup_replay_retained_receipt_identity_binding_receipt_replay_identity(
    evidence: RecoveryStartupReplayRetainedReceiptIdentityBindingReceiptReplayEvidence,
    *,
    destination_path: str | os.PathLike[str],
) -> RecoveryStartupReplayRetainedReceiptIdentityBindingReceiptReplayIdentityStoreEvidence:
    """Persist the minimal canonical identity of one compatible P95 replay."""
    if not isinstance(evidence, RecoveryStartupReplayRetainedReceiptIdentityBindingReceiptReplayEvidence):
        raise ValueError("P95 replay evidence has an incompatible type")
    if evidence.evidence_state != P95_EVIDENCE_STATE:
        raise ValueError("P95 replay evidence state is incompatible")
    if evidence.automatic_control_allowed is not False:
        raise ValueError("P95 replay evidence must not grant automatic-control authority")
    if any(getattr(evidence, flag, None) is not True for flag in _REQUIRED_VERIFICATION_FLAGS):
        raise ValueError("P95 replay evidence verification flags are incomplete")

    values: dict[str, object] = {}
    for field, kind in _FIELDS:
        raw = getattr(evidence, field, None)
        values[field] = (
            _positive_int(raw, field=f"P95 {field}")
            if kind == "int"
            else _sha256(raw, field=f"P95 {field}")
        )

    payload = {**values, "p95_evidence_state": P95_EVIDENCE_STATE}
    ...
    if stored != encoded:
        raise ValueError("stored P95 replay identity differs from published bytes")
    if hashlib.sha256(stored).hexdigest() != encoded_sha:
        raise ValueError("stored P95 replay identity SHA-256 mismatch")

    return RecoveryStartupReplayRetainedReceiptIdentityBindingReceiptReplayIdentityStoreEvidence(
        **values,
