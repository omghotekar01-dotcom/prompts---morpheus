...
class DistributionCalibrationReplicationReport:
    ...
    require_all_holdouts_accepted: bool
    ...
    def as_dict(self) -> dict[str, object]:
        return {
            ...
            "require_all_holdouts_accepted": self.require_all_holdouts_accepted,
...
def evaluate_distribution_calibration_replication(
    ...
    *,
    ...
    require_all_holdouts_accepted: bool = True,
    ...
    magnitudes are not directly comparable. It also refuses mixed measurement protocols so a replication result
    cannot silently pool incompatible experimental procedures.

    The caller supplies the allowed cross-machine MAPE spread. Passing means only that the supplied reports meet
    that declared replication criterion (and, by default, that every constituent holdout met its own declared
    acceptance limits).
    """
    ...
    all_accepted = all(report.acceptance_passed for report in items)
    replication_passed = spread <= max_allowed_machine_mape_spread and (
        all_accepted or not require_all_holdouts_accepted
    )
    ...
    return DistributionCalibrationReplicationReport(
        ...
        require_all_holdouts_accepted=require_all_holdouts_accepted,
        all_holdouts_accepted=all_accepted,
