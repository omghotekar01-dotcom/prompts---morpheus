"""Persist the minimum verified P105 canonical P104-replay identity locally.

P106 closes only the local-retention seam left explicit by P105. A caller supplies
verified P105 replay evidence; this gate validates that evidence contract, reduces
...
EVIDENCE_STATE = P105_EVIDENCE_STATE + "_IDENTITY_STORED"
TRUTH_BOUNDARY = (
    "This gate proves only that compatible verified P105 replay evidence was reduced to a strict canonical local identity record, "
    ...
    "monotonic trust anchor. This gate does not authenticate P105 evidence or the filesystem, prove freshness/latest/global/monotonic "
    "head truth, prevent rollback/replay or coordinated replacement, rerun P104/P105 or dependencies, authorize startup or mutation, "
...
class RecoveryP105ReplayIdentityStoreEvidence:
    sequence: int
    lineage_sha256: str
    ...
    p105_evidence_state_verified: bool
    p105_verification_flags_verified: bool
...
def store_p105_replay_identity(
    evidence: RecoveryP104ReplayEvidence,
    *,
    destination_path: str | os.PathLike[str],
) -> RecoveryP105ReplayIdentityStoreEvidence:
    """Persist the minimal canonical identity of one compatible P105 replay."""
    if not isinstance(evidence, RecoveryP104ReplayEvidence):
        raise ValueError("P105 replay evidence has an incompatible type")
    if evidence.evidence_state != P105_EVIDENCE_STATE:
        raise ValueError("P105 replay evidence state is incompatible")
    if evidence.automatic_control_allowed is not False:
        raise ValueError("P105 replay evidence must not grant automatic-control authority")
    if any(getattr(evidence, flag, None) is not True for flag in _REQUIRED_VERIFICATION_FLAGS):
        raise ValueError("P105 replay evidence verification flags are incomplete")

    values: dict[str, object] = {}
    for field, kind in _FIELDS:
        raw = getattr(evidence, field, None)
        values[field] = _positive_int(raw, field=f"P105 {field}") if kind == "int" else _sha256(raw, field=f"P105 {field}")

    payload = {**values, "p105_evidence_state": P105_EVIDENCE_STATE}
    ...
    if stored != encoded:
        raise ValueError("stored P105 replay identity differs from published bytes")
    if hashlib.sha256(stored).hexdigest() != encoded_sha:
        raise ValueError("stored P105 replay identity SHA-256 mismatch")

    return RecoveryP105ReplayIdentityStoreEvidence(
        **values,
