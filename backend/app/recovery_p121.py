"""Persist the minimum verified P120 canonical P119-replay identity locally.

P121 closes only the local-retention seam left explicit by P120. A caller supplies
verified P120 replay evidence; this gate validates that evidence contract, reduces
...
from .recovery_p120 import (
    EVIDENCE_STATE as P120_EVIDENCE_STATE,
    RecoveryP119ReplayEvidence,
    _FIELDS as P120_RECEIPT_FIELDS,
)

EVIDENCE_STATE = P120_EVIDENCE_STATE + "_IDENTITY_STORED"
TRUTH_BOUNDARY = (
    "This gate proves only that compatible verified P120 replay evidence was reduced to a strict canonical local identity record, "
    ...
    "monotonic trust anchor. This gate does not authenticate P120 evidence or the filesystem, prove freshness/latest/global/monotonic "
    "head truth, prevent rollback/replay or coordinated replacement, rerun P119/P120 or dependencies, authorize startup or mutation, "
...
_FIELDS = (
    *P120_RECEIPT_FIELDS,
    ("p119_receipt_payload_sha256", "sha"),
...
class RecoveryP120ReplayIdentityStoreEvidence:
    sequence: int
    lineage_sha256: str
    ...
    p120_evidence_state_verified: bool
    p120_verification_flags_verified: bool
...
def store_p120_replay_identity(
    evidence: RecoveryP119ReplayEvidence,
    *,
    destination_path: str | os.PathLike[str],
) -> RecoveryP120ReplayIdentityStoreEvidence:
    """Persist the minimal canonical identity of one compatible P120 replay."""
    if not isinstance(evidence, RecoveryP119ReplayEvidence):
        raise ValueError("P120 replay evidence has an incompatible type")
    if evidence.evidence_state != P120_EVIDENCE_STATE:
        raise ValueError("P120 replay evidence state is incompatible")
    if evidence.automatic_control_allowed is not False:
        raise ValueError("P120 replay evidence must not grant automatic-control authority")
    if any(getattr(evidence, flag, None) is not True for flag in _REQUIRED_VERIFICATION_FLAGS):
        raise ValueError("P120 replay evidence verification flags are incomplete")

    values: dict[str, object] = {}
    for field, kind in _FIELDS:
        raw = getattr(evidence, field, None)
        values[field] = (
            _positive_int(raw, field=f"P120 {field}")
            if kind == "int"
            else _sha256(raw, field=f"P120 {field}")
        )

    payload = {**values, "p120_evidence_state": P120_EVIDENCE_STATE}
    ...
    if stored != encoded:
        raise ValueError("stored P120 replay identity differs from published bytes")
    if hashlib.sha256(stored).hexdigest() != encoded_sha:
        raise ValueError("stored P120 replay identity SHA-256 mismatch")

    return RecoveryP120ReplayIdentityStoreEvidence(
        **values,
