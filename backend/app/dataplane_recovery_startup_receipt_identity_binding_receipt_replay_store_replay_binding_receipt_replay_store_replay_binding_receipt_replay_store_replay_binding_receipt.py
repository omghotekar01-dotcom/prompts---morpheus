"""Canonicalize verified P93 replay/retained-receipt composition evidence.

P94 is the serialization boundary for the P93 read-only composition result. It
validates the complete P93 contract, emits strict canonical JSON bytes, and
...
EVIDENCE_STATE = (
    ...
    "RECEIPT_REPLAY_IDENTITY_STORED_REPLAY_BINDING_RECEIPT_REPLAY_VERIFIED_IDENTITY_STORED_REPLAY_BINDING_RECEIPT_CANONICAL"
)
SCHEMA = "morpheus.recovery.p94.replay-retained-receipt-identity-binding-receipt.v1"
TRUTH_BOUNDARY = (
    "This read-only portability gate proves only that a compatible supplied P93 evidence object satisfied its exported verification contract "
    "and was serialized into deterministic canonical JSON bytes whose exact byte length and SHA-256 are reported. It does not authenticate P93 "
    "or the receipt bytes, rerun P90/P92/P93 or dependencies, establish freshness/latest/global/monotonic head truth, prevent replay or coordinated "
...
_P93_FLAGS = ("p90_contract_verified", "p92_contract_verified", "cross_evidence_identity_verified")
_FIELDS = (
    ("sequence", "int"),
    ("lineage_sha256", "sha"),
...
class RecoveryStartupReplayRetainedReceiptIdentityBindingReceiptEvidence:
    sequence: int
    lineage_sha256: str
...
def canonicalize_recovery_startup_replay_retained_receipt_identity_binding_receipt(
    evidence: RecoveryStartupReplayRetainedIdentityBindingReceiptReplayIdentityStoredReplayBindingEvidence,
) -> RecoveryStartupReplayRetainedReceiptIdentityBindingReceiptEvidence:
    """Serialize P93 evidence canonically without granting startup authority."""
    if not isinstance(evidence, RecoveryStartupReplayRetainedIdentityBindingReceiptReplayIdentityStoredReplayBindingEvidence):
        raise ValueError("P93 replay/retained-receipt identity binding evidence has an incompatible type")
    if evidence.evidence_state != P93_EVIDENCE_STATE:
        raise ValueError("P93 evidence state is incompatible")
    if evidence.automatic_control_allowed is not False:
        raise ValueError("P93 evidence must not grant automatic-control authority")
    if any(getattr(evidence, flag, None) is not True for flag in _P93_FLAGS):
        raise ValueError("P93 verification flags are incomplete")

    values: dict[str, object] = {}
    for field, kind in _FIELDS:
        raw = getattr(evidence, field)
        values[field] = _positive_int(raw, field=f"P93 {field}") if kind == "int" else _sha256(raw, field=f"P93 {field}")
    ...
    return RecoveryStartupReplayRetainedReceiptIdentityBindingReceiptEvidence(
        **values,
        payload=payload,
        payload_sha256=digest,
        payload_size_bytes=len(payload),
        p93_contract_verified=True,
