...
| P2 | C++20 primitive laboratory | TESTED_EXPANDED | Robin Hood hash, real B+ tree, sorted array, trie, bitmap baseline, CSR graph, versioned slot, and dependency-free partitioned compressed bitmap with CTest coverage; B+ deletion remains correctness-first rebuild; compressed bitmap currently uses sorted 16-bit array containers rather than adaptive Roaring array/bitset/run containers |
...
GitHub Actions run `33109600417` at commit `aad8814606aa7b67afbd692e1ca0064c2b759a53` passed backend Linux/Windows, frontend build, core Linux/MSVC, sanitizer, calibration smoke and paired baseline smoke tracks. Later commits, including the compressed bitmap milestone, must be independently checked before being called green.
...
- The new `CompressedBitmap` is genuinely partitioned storage (high 16-bit key -> sorted low 16-bit array), but is not yet a full adaptive Roaring implementation with dense bitset/run containers.
...
5. evolve compressed bitmap to measured adaptive array/bitset/run containers and connect CSR graph to generic artifact codegen where semantics justify it;
